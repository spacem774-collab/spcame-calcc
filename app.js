(function () {
  ...
  function getRequestMailUrl(service) {
    var body = buildRequestEmailBody();
    var subjectEnc = encodeURIComponent(REQUEST_SUBJECT);
    var bodyEnc = encodeURIComponent(body);
    if (service === 'gmail') return 'https://mail.google.com/mail/?view=cm&fs=1&to=' + encodeURIComponent(REQUEST_TO) + '&su=' + subjectEnc + '&body=' + bodyEnc;
    if (service === 'yandex') return 'https://mail.yandex.ru/compose/?to=' + encodeURIComponent(REQUEST_TO) + '&subject=' + subjectEnc + '&body=' + bodyEnc;
    if (service === 'mailru') return 'https://e.mail.ru/compose/?to=' + encodeURIComponent(REQUEST_TO) + '&subject=' + subjectEnc + '&body=' + bodyEnc;
    return '#';
  }

  function updateRequestLinks() {
    if (requestOpenGmail) requestOpenGmail.href = getRequestMailUrl('gmail');
    if (requestOpenYandex) requestOpenYandex.href = getRequestMailUrl('yandex');
    if (requestOpenMailru) requestOpenMailru.href = getRequestMailUrl('mailru');
  }

  function openMailWithBody(service, clickedEl) {
    if (history.length === 0) {
      if (clickedEl) {
        var t = clickedEl.textContent;
        clickedEl.textContent = 'Сначала выполните расчёты';
        setTimeout(function () { clickedEl.textContent = t; }, 2500);
      }
      return;
    }
    var url = getRequestMailUrl(service);
  ...
  function copyRequestToClipboard() {
    if (history.length === 0) {
      if (requestCopyBody) {
        ...
        requestCopyBody.textContent = 'Сначала выполните расчёты';
        setTimeout(function () { requestCopyBody.textContent = t; }, 2500);
      }
      return;
    }
    var body = buildRequestEmailBody();
  ...
  if (requestOpenGmail) requestOpenGmail.addEventListener('click', function (e) { e.preventDefault(); openMailWithBody('gmail', e.currentTarget); });
  if (requestOpenYandex) requestOpenYandex.addEventListener('click', function (e) { e.preventDefault(); openMailWithBody('yandex', e.currentTarget); });
  if (requestOpenMailru) requestOpenMailru.addEventListener('click', function (e) { e.preventDefault(); openMailWithBody('mailru', e.currentTarget); });
